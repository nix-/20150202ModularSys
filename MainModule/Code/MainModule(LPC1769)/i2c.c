...
#define CONSET_I2EN		0x00000040  /* I2C Control Set Register */
#define CONSET_AA		0x00000004
#define CONSET_SI		0x00000008
#define CONSET_STO		0x00000010
#define CONSET_STA		0x00000020

#define CONCLR_AAC		0x00000004  /* I2C Control clear Register */
#define CONCLR_SIC		0x00000008
#define CONCLR_STAC		0x00000020
#define CONCLR_I2ENC	0x00000040

#define DAT_I2C			0x00000000  /* I2C Data Reg */
#define ADR_I2C			0x00000000  /* I2C Slave Address Reg */
#define SCLH_SCLH		58  		/* I2C SCL Duty Cycle High Reg */
#define SCLL_SCLL		57  		/* I2C SCL Duty Cycle Low Reg */
#define SCLH_HS_SCLH	0x00000020  /* Fast Plus I2C SCL Duty Cycle High Reg */
#define SCLL_HS_SCLL	0x00000020  /* Fast Plus I2C SCL Duty Cycle Low Reg */
...
void I2C1_IRQHandler(void)
{
	...
	case 0x08:
		 ...
		 */
		WrIndex = 0;
		LPC_I2C1->DAT = I2CMasterBuffer[WrIndex++];
		//LPC_I2C1->I2CONSET = I2CONSET_AA;
		LPC_I2C1->CONCLR = (CONCLR_SIC | CONCLR_STAC);
	...
	case 0x28:
		...
		else
		{
			if ( I2CReadLength != 0 )
			{
				...
				LPC_I2C1->CONSET = CONSET_STA;	/* Set Repeated-start flag */
			}
			else
			{
				I2CMasterState = I2CSTATE_ACK;
				LPC_I2C1->CONSET = CONSET_STO;      /* Set Stop flag */
			}
		}
		LPC_I2C1->CONCLR = CONCLR_SIC;
		break;
	...
	case 0x50:
		 ...
		 */
		I2CSlaveBuffer[RdIndex++] = LPC_I2C1->DAT;
	...
	case 0x58:
		...
		I2CMasterState = I2CSTATE_ACK;
		LPC_I2C1->CONSET = CONSET_STO;	/* Set Stop flag */
		LPC_I2C1->CONCLR = CONCLR_SIC;	/* Clear SI flag */
		break;
...
static uint32_t I2CStart( void )
{
	...
	LPC_I2C1->CONSET = CONSET_STA;	/* Set Start flag */
...
static uint32_t I2CStop( void )
{
	...
	while((LPC_I2C1->CONSET & CONSET_STO) && (timeout < MAX_TIMEOUT))
	{
		timeout++;
	}
	return (timeout >= MAX_TIMEOUT);
...
uint32_t I2CInit( uint32_t I2cMode )
{
	...
	/*--- Clear flags ---*/
	LPC_I2C1->CONCLR = CONCLR_AAC | CONCLR_SIC | CONCLR_STAC | CONCLR_I2ENC;
...
#else
	LPC_I2C1->SCLL   = 16;  // i2c freq = (100,000,000/8)/ (32) = 390.63khz
	LPC_I2C1->SCLH   = 16;
#endif
